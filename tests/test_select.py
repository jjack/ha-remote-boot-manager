...
async def test_async_setup_entry_duplicate_host(hass):
    """Test signal callback adds entity again (HA handles duplicates)."""
    mock_entry = MagicMock()
    mock_manager = MagicMock()
    mac = "00:11:22:33:44:55"
    ...
    with patch("custom_components.grubstation.select.async_dispatcher_connect") as mock_connect:
        ...
        # In current implementation, it calls async_add_entities again
        assert async_add_entities.call_count == 2
...
async def test_select_init_model_name(hass):
    ...
    select2 = GrubStationManagerSelect(coordinator2)
...
async def test_select_properties(hass):
    ...
    select_missing = GrubStationManagerSelect(coordinator_missing)
