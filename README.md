...
At the core of this integration is the `grubstation`. It performs three functions: 1) parse grub boot options and send them to Home Assistant via webhook; 2) install a grub configuration script into `/etc/grub.d`
...
### Basic Agent Setup:
1. Download the latest [grubstation](https://github.com/jjack/grubstation/releases/latest).
2. Install the agent on your target host.
3. Run `grubstation setup` to auto-detect as much of the configuration as possible (GRUB, init system, network info, etc.) and configure it with GRUB and your init system. This uses the `webhook_id` you saved during the integration setup.
